...
import { getRendimientoEsperado } from '../utils/colombia';
import {
  DollarSign, TrendingUp, Package, Activity,
  Receipt, ShoppingCart, BarChart3, Plus, Trash2, Edit2,
  ArrowUpRight, ArrowDownRight, Minus, AlertTriangle, Info
} from 'lucide-react';
...
// Helper: muestra valor con contexto o estado vacio
function MetricValue({ value, formatted, context, trend, empty = 'Sin datos aun' }) {
  if (value === 0 || value === null || value === undefined) {
    return (
      <div>
        <p className="text-lg font-bold text-tierra-300">{formatted || '—'}</p>
        <p className="text-[11px] text-tierra-400 mt-0.5">{empty}</p>
      </div>
    );
  }
  const TrendIcon = trend === 'up' ? ArrowUpRight : trend === 'down' ? ArrowDownRight : Minus;
  const trendColor = trend === 'up' ? 'text-campo-600' : trend === 'down' ? 'text-red-500' : 'text-tierra-400';
  return (
    <div>
      <div className="flex items-center gap-1">
        <p className="text-lg font-bold text-tierra-900">{formatted}</p>
        {trend && <TrendIcon className={`w-4 h-4 ${trendColor}`} />}
      </div>
      {context && <p className="text-[11px] text-tierra-500 mt-0.5">{context}</p>}
    </div>
  );
}
...
export default function CosechaDetallePage() {
  ...
  const roi = costoTotal > 0 ? ((ingresoTotal - costoTotal) / costoTotal) * 100 : 0;
  const areaSembrada = parseFloat(cosecha.area_sembrada || 0);
  const rendimientoPorHa = areaSembrada > 0 ? produccionTotal / areaSembrada : 0;
  const margen = ingresoTotal > 0 ? (utilidad / ingresoTotal) * 100 : 0;
  const perdidas = parseFloat(cosecha.perdidas || 0);
  const porcentajePerdidas = produccionTotal + perdidas > 0 ? (perdidas / (produccionTotal + perdidas)) * 100 : 0;

  // Rendimiento esperado para contexto
  const rendEsperado = getRendimientoEsperado(cosecha.variedad_papa);
  const produccionEsperada = areaSembrada > 0 ? rendEsperado * areaSembrada : 0;
  const porcentajeCumplimiento = produccionEsperada > 0 && produccionTotal > 0
    ? (produccionTotal / produccionEsperada) * 100 : 0;

  // Alertas inteligentes
  const alertas = [];
  if (porcentajePerdidas > 15) alertas.push({ tipo: 'danger', msg: `Perdidas altas: ${porcentajePerdidas.toFixed(1)}% de la produccion` });
  if (costoPorKg > 0 && ingresoTotal > 0) {
    const precioVentaPromKg = produccionTotal > 0 ? ingresoTotal / produccionTotal : 0;
    if (costoPorKg > precioVentaPromKg * 0.9) alertas.push({ tipo: 'warning', msg: 'Costos cercanos al precio de venta. Margen muy bajo.' });
  }
  if (roi < -10 && costoTotal > 0 && ingresoTotal > 0) alertas.push({ tipo: 'danger', msg: `ROI negativo: ${roi.toFixed(1)}%. La cosecha genera perdidas.` });
  if (porcentajeCumplimiento > 0 && porcentajeCumplimiento < 60) alertas.push({ tipo: 'warning', msg: `Rendimiento bajo: ${porcentajeCumplimiento.toFixed(0)}% del esperado para ${cosecha.variedad_papa}` });
  ...
  return (
    <div className="animate-fade-in">
      ...
      <PageHeader
        ...
        action={<span className={`text-xs font-semibold px-3 py-1 rounded-full ${est.color}`}>{est.label}</span>}
      />
        ...
        <div className="space-y-2 mb-5">
          {alertas.map((a, i) => (
            <div key={i} className={`flex items-center gap-3 p-3 rounded-xl text-sm ${
              a.tipo === 'danger' ? 'bg-red-50 text-red-700 border border-red-200' : 'bg-cosecha-50 text-cosecha-800 border border-cosecha-200'
            }`}>
      ...
      {/* ════ NIVEL 1: METRICAS PRINCIPALES (grandes, destacadas) ════ */}
      <div className="grid grid-cols-3 gap-3 mb-4">
        <div className={`card p-5 ${ingresoTotal > 0 ? 'border-l-4 border-l-campo-500' : ''}`}>
          ...
          <MetricValue
            value={ingresoTotal}
            formatted={formatCOP(ingresoTotal)}
            context={ingresoTotal > 0 ? `${(cosecha.ventas || []).length} ventas registradas` : undefined}
            trend={ingresoTotal > 0 ? 'up' : undefined}
            empty="Sin ventas registradas"
          />
        </div>

        <div className={`card p-5 ${costoTotal > 0 ? 'border-l-4 border-l-tierra-400' : ''}`}>
          <div className="flex items-center gap-2 mb-2">
          ...
          <MetricValue
            value={costoTotal}
            formatted={formatCOP(costoTotal)}
            context={costoTotal > 0 ? `${(cosecha.gastos || []).length} gastos + ${(cosecha.actividades || []).filter(a => parseFloat(a.costo) > 0).length} actividades` : undefined}
        ...
        <div className={`card p-5 ${utilidad !== 0 ? (utilidad > 0 ? 'border-l-4 border-l-campo-500 bg-campo-50/30' : 'border-l-4 border-l-red-400 bg-red-50/30') : ''}`}>
          ...
          <MetricValue
            value={ingresoTotal > 0 || costoTotal > 0 ? utilidad : null}
            formatted={formatCOP(utilidad)}
            context={utilidad > 0 ? `Positiva — ganancia neta` : utilidad < 0 ? `Negativa — hay perdida` : undefined}
            trend={utilidad > 0 ? 'up' : utilidad < 0 ? 'down' : undefined}
            empty="Registra ingresos y gastos para calcular"
          />
        </div>
      </div>

      {/* ════ NIVEL 2: METRICAS SECUNDARIAS (pequenas, con contexto) ════ */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
        <div className="card p-3">
          <p className="text-[11px] text-tierra-500 uppercase tracking-wide mb-1">ROI</p>
          {costoTotal > 0 && ingresoTotal > 0 ? (
            <>
              <p className={`text-base font-bold ${roi >= 0 ? 'text-campo-700' : 'text-red-600'}`}>
                {roi >= 0 ? '+' : ''}{roi.toFixed(1)}%
              </p>
              <p className="text-[10px] text-tierra-400">Retorno sobre inversion (utilidad / costos)</p>
            </>
          ) : (
            <>
        ...
        <div className="card p-3">
          ...
          {costoPorKg > 0 ? (
            <>
              ...
              <p className="text-[10px] text-tierra-400">Costo total / produccion total</p>
            </>
          ) : (
            <>
              <p className="text-base font-bold text-tierra-300">—</p>
              <p className="text-[10px] text-tierra-400">{produccionTotal === 0 ? 'Registra produccion primero' : 'Sin costos aun'}</p>
            </>
          )}
        </div>
        <div className="card p-3">
          <p className="text-[11px] text-tierra-500 uppercase tracking-wide mb-1">Rendimiento/ha</p>
          {rendimientoPorHa > 0 ? (
            <>
              <p className="text-base font-bold text-cosecha-700">{formatKg(rendimientoPorHa)}</p>
              <p className="text-[10px] text-tierra-400">
                {produccionEsperada > 0
                  ? `${porcentajeCumplimiento.toFixed(0)}% del esperado (~${formatKg(rendEsperado)}/ha)`
                  : 'Produccion / area sembrada'
                }
              </p>
            </>
          ) : (
            <>
              <p className="text-base font-bold text-tierra-300">—</p>
              <p className="text-[10px] text-tierra-400">{areaSembrada === 0 ? 'Sin area definida' : 'Sin produccion aun'}</p>
            </>
          )}
        </div>
        <div className="card p-3">
          <p className="text-[11px] text-tierra-500 uppercase tracking-wide mb-1">Margen</p>
          {ingresoTotal > 0 ? (
            <>
              <p className={`text-base font-bold ${margen >= 0 ? 'text-campo-700' : 'text-red-600'}`}>
                {margen.toFixed(1)}%
              </p>
              <p className="text-[10px] text-tierra-400">Utilidad como % de ingresos</p>
            </>
          ) : (
            <>
              <p className="text-base font-bold text-tierra-300">—</p>
              <p className="text-[10px] text-tierra-400">Registra ventas para calcular</p>
            </>
          )}
        </div>
      </div>

      {/* Produccion con contexto */}
      {produccionTotal > 0 && produccionEsperada > 0 && (
        <div className="card p-3 mb-5 flex items-center gap-3 bg-campo-50/50">
          <Info className="w-4 h-4 text-campo-600 shrink-0" />
          <p className="text-xs text-campo-700">
            <strong>Produccion: {formatKg(produccionTotal)}</strong>
            {' '}(esperado: ~{formatKg(produccionEsperada)} para {areaSembrada} ha de {cosecha.variedad_papa})
            {' — '}{porcentajeCumplimiento.toFixed(0)}% del objetivo
          </p>
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          <div className="card p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-display font-bold text-base">Produccion</h3>
            ...
            {produccionTotal === 0 && !cosecha.produccion_primera ? (
              <div className="text-center py-4">
                <Package className="w-8 h-8 text-tierra-300 mx-auto mb-2" />
                <p className="text-sm text-tierra-500">Sin datos de produccion</p>
                <p className="text-xs text-tierra-400">Haz click en "Editar" para registrar la produccion cuando coseches</p>
              </div>
            ) : (
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-sm">
                ...
                <div>
                  <span className="text-tierra-500 text-xs">Perdidas</span>
                  <p className={`font-semibold ${perdidas > 0 ? 'text-red-600' : ''}`}>
                    {formatKg(perdidas)}
                    {porcentajePerdidas > 0 && <span className="text-[10px] ml-1">({porcentajePerdidas.toFixed(1)}%)</span>}
                  </p>
                </div>
          ...
          {pieData.length > 0 && (
            <div className="card p-5">
              <h3 className="font-display font-bold text-base mb-4">Distribucion de costos</h3>
        ...
        <div>
          ...
          {(cosecha.actividades || []).length === 0 ? (
            <div className="text-center py-10">
              <Activity className="w-8 h-8 text-tierra-300 mx-auto mb-2" />
              <p className="text-sm text-tierra-500">Sin actividades registradas</p>
              <p className="text-xs text-tierra-400">Registra siembras, fertilizaciones, riegos y mas</p>
            </div>
          ) : (
      ...
      {tab === 'gastos' && (
        <div>
          ...
          {(cosecha.gastos || []).length === 0 ? (
            <div className="text-center py-10">
              <Receipt className="w-8 h-8 text-tierra-300 mx-auto mb-2" />
              <p className="text-sm text-tierra-500">Sin gastos registrados</p>
              <p className="text-xs text-tierra-400">Registra insumos, mano de obra, transporte y mas</p>
            </div>
          ) : (
            <div className="space-y-2">
      ...
      {tab === 'ventas' && (
        <div>
          ...
          {(cosecha.ventas || []).length === 0 ? (
            <div className="text-center py-10">
              <ShoppingCart className="w-8 h-8 text-tierra-300 mx-auto mb-2" />
              <p className="text-sm text-tierra-500">Sin ventas registradas</p>
              <p className="text-xs text-tierra-400">Registra ventas por calidad, cliente y precio</p>
            </div>
          ) : (
            <div className="space-y-2">
      ...
      <Modal isOpen={modal.open && modal.type === 'actividad'} onClose={() => setModal({ open: false })}
        title={modal.editing ? 'Editar actividad' : 'Nueva actividad'} maxWidth="max-w-2xl">
        <form onSubmit={saveItem} className="space-y-4">
          ...
          <div><label className="label">Descripcion</label>
            <textarea name="descripcion" value={form.descripcion} onChange={onChange} className="input-field" rows={2} /></div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="label">Producto usado</label>
              <input name="producto_usado" value={form.producto_usado} onChange={onChange} className="input-field" placeholder="Ej: Fertilizante 10-30-10" /></div>
      ...
      <Modal isOpen={modal.open && modal.type === 'gasto'} onClose={() => setModal({ open: false })}
        title={modal.editing ? 'Editar gasto' : 'Nuevo gasto'} maxWidth="max-w-2xl">
        <form onSubmit={saveItem} className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div><label className="label">Tipo *</label>
              <select name="tipo" value={form.tipo} onChange={onChange} className="input-field">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div><label className="label">Total</label>
              <input name="valor_total" type="number" step="100" value={form.valor_total} onChange={onChange} className="input-field bg-tierra-50" readOnly /></div>
          </div>
      ...
      <Modal isOpen={modal.open && modal.type === 'venta'} onClose={() => setModal({ open: false })}
        title={modal.editing ? 'Editar venta' : 'Nueva venta'} maxWidth="max-w-2xl">
        <form onSubmit={saveItem} className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
      ...
      <Modal isOpen={prodModal} onClose={() => setProdModal(false)} title="Actualizar produccion" maxWidth="max-w-2xl">
